    ...
    <div class="container mb-10">
      <div class="card shadow-sm mt-5">
        <div class="card-header">
          <h3 class="card-title fw-bold">List of Product</h3>
          ...
          <div class="py-5">
            <div class="table-responsive">
              <table class="table table-row-bordered table-row-gray-200 gy-4">
                ...
                <tbody>
                  <tr
                    v-for="(produc, produc_index) in product.data"
                    :key="produc_index"
                  >
                    <td class="text-center">{{ produc_index + 1 }}</td>
                    <td class="text-center">{{ produc.name }}</td>
                    <td class="text-center">{{ produc.description }}</td>
                ...
                <ul class="pagination">
                  <li class="page-item">
                    <button
                      ...
                      @click="
                        product.prev_page_url && list(product.prev_page_url)
                      "
                    >
                      Previous
                    </button>
                  ...
                  <li class="page-item">
                    <button
                      ...
                      @click="
                        product.next_page_url && list(product.next_page_url)
                      "
                    >
                      Next
                    </button>
...
export default {
  ...
  methods: {
    ...
    edit(produc) {
      this.produc.id = produc.id
      this.produc.name = produc.name
      this.produc.description = produc.description
    },
    update() {
      this.loading()

      this.$axios
        .put('/api/product-update/' + this.produc.id, {
          name: this.produc.name,
          description: this.produc.description,
        })
        .then((result) => {
          Swal.fire({
