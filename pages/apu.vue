    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6 bg-white">
      ...
      <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
          ...
          <li class="breadcrumb-item text-muted">APU</li>
          <!--end::Item-->
        </ul>
        <!--end::Page title-->
      </div>
          ...
          <div class="py-5">
            <div class="table-responsive">
              <table class="table table-row-bordered table-row-gray-200 gy-4">
                ...
                <tbody>
                  <tr v-for="(p_apu, p_apu_index) in apu.data" :key="p_apu_index">
                    <td class="text-center">{{ apu.from + p_apu_index }}</td>
                    <td class="text-center">{{ p_apu.name }}</td>
                    ...
                    <td class="d-flex justify-content-center">
                      ...
                      <button class="btn btn-sm btn-light" v-on:click="remove(p_apu.id)">
                        <i class="bi bi-trash-fill text-primary"></i>
                      </button>
                    </td>
                  </tr>
                  <tr v-if="apu.data.length < 1">
                    <td colspan="8">
                ...
                <ul class="pagination">
                  <li class="page-item">
                    <button type="button" class="page-link" :class="{
                      disabled: !apu.prev_page_url,
                    }" @click="apu.prev_page_url && list(apu.prev_page_url)">
                      Previous
                    </button>
                  </li>
                  ...
                  <li class="page-item">
                    <button type="button" class="page-link" :class="{
                      disabled: !apu.next_page_url,
                    }" @click="apu.next_page_url && list(apu.next_page_url)">
                      Next
                    </button>
                  </li>
                </ul>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h3 v-if="modal_create" class="modal-title">Add APU</h3>
            <h3 v-else class="modal-title">Edit APU</h3>
          ...
          <div class="modal-body">
            <form @submit.prevent="submit">
              <div class="form-group mb-3">
                ...
                <input type="text" class="form-control" v-model="p_apu.name" :class="{
  ...
  methods: {
    list(paginate) {
      ...
      this.$axios
        ...
        .then((response) => {
          this.apu = response.data.data;
          this.current_page = this.apu.current_page;
          Swal.close();
        })
    ...
    directPage: debounce(function () {
      if (this.current_page < 1) {
        this.current_page = 1;
      } else if (this.current_page > this.apu.last_page) {
        this.current_page = this.apu.last_page;
      }
      let url = new URL(this.apu.first_page_url);
    ...
    create() {
      ...
      this.$axios
        .post("/api/apu-create", {
          name: this.p_apu.name,

          //description: this.p_apu.description,
        })
        .then((response) => {
    ...
    edit(p_apu) {
      ...
      this.p_apu.id = p_apu.id;
      this.p_apu.name = p_apu.name;

      //this.p_apu.description = p_apu.description;
    },
    update() {
      this.loading();

      this.$axios
        .put("/api/apu-update/" + this.p_apu.id, {
          name: this.p_apu.name,
      ...
      Swal.fire({
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$axios.delete("/api/apu-delete/" + id).then((response) => {
              toastr.success(response.data.message);
              this.list();
            });
          }
    ...
    clearForm() {
      this.p_apu.id = null;
      this.p_apu.name = null;

      //this.p_apu.description = null;
