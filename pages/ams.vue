				...
				<div class="card-header">
					...
					<div class="card-toolbar">
						<button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
							data-bs-target="#modal" @click="add()">Add AMS</button>
					</div>
				</div>
				<div class="card-body">
					<div class="row d-flex align-items-center">
						...
						<div class="position-relative me-md-2">
							...
							<!--end::Svg Icon-->
							<input type="text" class="form-control form-control-solid ps-10" initial="search"
									...
									<tr v-for="(p_ams, p_ams_index) in ams.data" :key="p_ams_index">
										...
										<td class="d-flex justify-content-center">
											...
											<button class="btn btn-sm btn-light" data-bs-toggle="modal"
												data-bs-target="#modal" @click="edit(p_ams)">
					...
					<div class="modal-body">
						<form @submit.prevent="submit">
							<div class="form-group mb-3">
								...
								<span v-if="errors.initial" class="error invalid-feedback">{{ errors.initial[0]
								}}</span>
							</div>
							<div class="form-group mb-3">
								<label class="form-label fw-bold">User</label>
								<select class="form-select" v-model="p_ams.user_id" :class="{
									'is-invalid': errors.user_id,
								}">
									<option selected disabled>Select User</option>
									<option v-for="user in users" v-bind:value="user.id"> {{ user.name }} </option>
								</select>
								<span v-if="errors.user_id" class="error invalid-feedback">{{ errors.user_id[0]
								}}</span>
							</div>
							<div class="row mt-10">
								<div class="col">
